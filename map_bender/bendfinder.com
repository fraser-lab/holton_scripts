...
EOF
    ...
    echo "SYMGEN $SG" |\
     pdbset xyzin $pdb xyzout P1.pdb > $logfile
...
if(-e "$user_fitparams") then
    ...
    foreach x ( x y z o B )
        ...
        awk 'NR==1{x=substr($1,1,1);mult=1;if(x!="B"){mult=1000.0};\
...
foreach h_k_l ( 0_0_0 `awk '{print $1"_"$2"_"$3}' hkl.txt` )
    ...
    echo "$hkl" |\
    cat - fitme |\
    awk 'NR==1{h=$1;k=$2;l=$3;pi=atan2(1,1)*4;h000=(h==0 && k==0 && l==0);next;}\
      {x=$1;y=$2;z=$3;++n;\
...
awk 'NR==1{a=$1;b=$2;c=$3}\
   /^a_dx/{print $0,sqrt($NF*$NF)*a/1000}\
   /^a_dy/{print $0,sqrt($NF*$NF)*b/1000}\
   /^a_dz/{print $0,sqrt($NF*$NF)*c/1000}' |\
sort -k5g > /dev/null
